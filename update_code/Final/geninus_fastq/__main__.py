...
from git_upload.update_code.Final.geninus_fastq.functions.update_db import UpdateFastQC

from git_upload.update_code.Final.upload_fastqc import UploadFastQC

from .functions import ParseInfo, MakeSampleSheet, Bcl2fastqRunner, RangerRunner, FastQCRunner, ParseFastQC, QueueController, SendEmail, UploadFastq, ToolConfig, SequencingConfig
...
class MakeFastq(ToolConfig, SequencingConfig):
    ...
    def run_single_fc(self, info: dict):
        ...
        # Update FastQC to database.
        self.dbupdater(fastqc_result_path, qc_dict, qc_table)
    ...
    def _initialize_module(self):
        try:
            ...
            self.dbupdater = UpdateFastQC(self.date)
            self.logger.info("FastQC updated to the database.")
